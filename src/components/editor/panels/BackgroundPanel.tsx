  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 pt-4 pb-3 border-b border-black/5">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Applies to the current page. White works great for print.</p>
      </div>
      <div className="flex-1 overflow-y-auto cv-scroll p-4">
        <h4 className="text-xs font-semibold text-muted-foreground mb-2">Solid colors</h4>
        <div className="grid grid-cols-8 gap-1.5">
          {SOLID_SWATCHES.map((c) => (
            <button
              key={c}
              onClick={() => applySolid(c)}
              className={`h-7 w-7 rounded-md border border-black/10 hover:scale-110 transition-transform ${
                bg?.type === 'solid' && bg.color === c ? 'ring-2 ring-[#00C4CC] ring-offset-1' : ''
              }`}
              style={{ background: c }}
        ...
        <h4 className="text-xs font-semibold text-muted-foreground mt-6 mb-2">Gradients</h4>
        <div className="grid grid-cols-4 gap-2">
          {GRADIENT_PRESETS.map((g, i) => (
            <button
              key={i}
              onClick={() => applyGradient(g.from, g.to, g.angle)}
              className={`h-16 rounded-lg border border-black/10 hover:scale-[1.03] transition-transform ${
                bg?.type === 'gradient' && bg.from === g.from && bg.to === g.to ? 'ring-2 ring-[#00C4CC] ring-offset-1' : ''
              }`}
